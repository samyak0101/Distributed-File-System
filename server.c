...
static int lookup_pinum_invalid = 0;
dir_ent_t *unlinkptr;
...
int Ser_MFS_Lookup(void* fs_img){
    ...
    // This is wrong I think; you can lookup directories or files
    // if(inode->type != 0){
    //   return -1;
    // }

    // int found = -1;     // Samy wtf is this shit
    // found -= 1; found += 1; found = -1;

    // loop through all directory pointers (30) and all dirents to find the file.
    for (int i = 0; i < DIRECT_PTRS; i++){
      // printf("entered for loop\n");
      if(inode->direct[i] < 0 || inode->direct[i] >= superblock->num_data){
        // continue, not break; otherwise we would have to reorder entire directory when unlinking
        continue;
      }
...
/* // server MFS UNLINK - Samy's version
...
int Ser_MFS_Unlink(void* fs_img){
  ...
  // if parent inum is invalid, return -1
  if(lookup_pinum_invalid == 1){
    lookup_pinum_invalid = 0;
    return -1;
  }
  // name not existing in parent directory is NOT a failure
  else if(inum == -1){
    return 0;
  }

  // Go to inode where file/directory to unlink was found
  void *addr = fs_img + (superblock->inode_region_addr * MFS_BLOCK_SIZE) + (inum * sizeof(inode_t));
  inode_t *inode = (inode_t*)addr;
  // if inode type is directory, MUST BE EMPTY to unlink, otherwise return -1
  if(inode->type == MFS_DIRECTORY){
    for(int i = 0; i < DIRECT_PTRS; i++){
      // if current directory data block pointer is unallocated, skip i
      if(inode->direct[i] == -1){
        continue;
      }
      addr = fs_img + (inode->direct[i] * MFS_BLOCK_SIZE);
      dir_ent_t *currentry = (dir_ent_t*)addr;
      // if any directory entry inside of any directory data block is being used, return -1
      for(int j = 0; j < MFS_BLOCK_SIZE/sizeof(dir_ent_t); j++){
        if(currentry[j].inum != -1){
          return -1;
        }
      }
    }
    // if this is reached, directory is empty and can be unlinked
    // deallocate every data block allocated for the directory in the data bitmap
    for(int i = 0; i < DIRECT_PTRS; i++){
      if(inode->direct[i] == -1){
        continue;
      }
      int bit = inode->direct[i] - superblock->data_region_addr;
      addr = fs_img + (superblock->data_bitmap_addr * MFS_BLOCK_SIZE);
      clear_bit((unsigned int*)addr, bit);
    }
    // deallocate the inode for the directory in the inode bitmap
    addr = fs_img + (superblock->inode_bitmap_addr * MFS_BLOCK_SIZE);
    clear_bit((unsigned int*)addr, inum);
  }
  // unlink for if inode type is regular file
  else{
    // deallocate data block allocated for the file in the data bitmap
    int bit = inode->direct[0] - superblock->data_region_addr;
    addr = fs_img + (superblock->data_bitmap_addr * MFS_BLOCK_SIZE);
    clear_bit((unsigned int*)addr, bit);
    // deallocate the inode for the file in the inode bitmap
    addr = fs_img + (superblock->inode_bitmap_addr * MFS_BLOCK_SIZE);
    clear_bit((unsigned int*)addr, inum);
  }
  // set directory entry where file/directory is stored to have an inum of -1 (unallocated)
  unlinkptr->inum = -1;
